...
<html lang="en">
	...
	<body>
		...
		<div id="mySidenav" class="sidenav">
		  ...
		  <a href="http://localhost/loged_employee.php">Home Page</a>
				...
				if(isset($_POST['conduct']) ){
					...
					if($_POST['kind']=="charge"){
						if($trans_am > $av_amount){
							...
							$profit= $trans_am + $profit  - ($trans_am * $supply / 100);
						...
						if($cr_limit < $c_d_amount + $trans_am){						
							...
							$query = "UPDATE simple_cust SET cur_due_amount=$c_d_amount WHERE ac_id=$cust_ac";
							$result = mysqli_query($db, $query);
							// update merchant's profit n debt
							$profit= $trans_am + $profit  - ($trans_am * $supply / 100);
							$debt= $debt + ($trans_am * $supply / 100);
					...
					if($flag==1){
						$query = "INSERT INTO transaction (tran_id, cust_name, merch_name, tran_date, tran_amount, kind, c_tran, emp_id) VALUES (NULL, ?, ?, NOW(), ?, ?, 'true', ?)";
						$stmt= mysqli_prepare($db, $query);
						mysqli_stmt_bind_param($stmt, "ssdss", $cname, $merch, $trans_am, $kind, $emp_id);
